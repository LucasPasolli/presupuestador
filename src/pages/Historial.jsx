...
const ESTADOS = {
  ...
  aprobado:   { label: 'Aprobado',   color: 'blue',   icon: ThumbsUp },
  pagado:     { label: 'Pagado',     color: 'green',  icon: CheckCircle2 },
...
// ─── Helper: descontar stock ───────────────────────────────────────────────
// Se llama cuando un presupuesto pasa a Aprobado o Pagado
...
function descontarStock(idPresupuesto) {
  ...
  for (const d of detalles) {
    ...
    if (prod.tieneMedidas && d.medida) {
      run(
        `UPDATE ProductoMedida SET cantidad = MAX(0, cantidad - ?) WHERE idProducto = ? AND medida = ?`,
        [d.cantidad, d.idProducto, d.medida]
      )
    } else {
      run(
        `UPDATE Producto SET cantidad = MAX(0, cantidad - ?) WHERE idProducto = ?`,
...
function PresupuestoDetalle({ presupuesto: presInit, onBack, onUpdated }) {
  ...
  const [modal,       setModal]       = useState(null)  // 'aprobar'|'pagar'|'rechazar'|null
  ...
  function cambiarEstado(nuevoEstado) {
    ...
    if (nuevoEstado === 'pagado' && esCC) {
      setErrorModal('Los presupuestos con CC se cobran desde la sección Saldos, no desde aquí.')
      return
    }
    if (nuevoEstado === 'pagado' && pres.estado !== 'aprobado' && pres.estado !== 'borrador') {
      setErrorModal('Solo se puede marcar como pagado un presupuesto borrador o aprobado.')
      return
    }
    ...
    // Descontar stock si pasa a aprobado o pagado (y antes no lo era)
    ...
    // Si es CC y pasa a aprobado → el saldo ya existe, no hace falta crear uno
    // Si es contado y pasa a pagado → el ingreso se refleja en estadísticas
    // porque cobradoReal suma efectivo+transf de presupuestos sin filtrar por estado
    // → ajustamos la query de Estadísticas para filtrar por estado pagado
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 animate-slide-up">
      ...
      <Card className="p-6">
        ...
        {puedeActuar && (
          <div className="mt-6 pt-5 border-t border-surface-700">
            <p className="text-surface-400 text-xs uppercase tracking-widest font-body mb-3">
            ...
            <div className="flex flex-wrap gap-2">
              {pres.estado === 'borrador' && (
                <>
                  ...
                  <Button size="sm" icon={ThumbsUp} onClick={() => setModal('aprobar')}>
                    Marcar como Aprobado
                  </Button>
                  {/* Pagado solo para contado (efectivo/transferencia/excepcion) */}
                  {!esCC && (
                    <Button size="sm" variant="secondary" icon={CheckCircle2} onClick={() => setModal('pagar')}>
                      Marcar como Pagado
                    </Button>
                  )}
              ...
              {pres.estado === 'aprobado' && !esCC && (
                <Button size="sm" icon={CheckCircle2} onClick={() => setModal('pagar')}>
                  Marcar como Pagado
                </Button>
              )}
              {pres.estado === 'aprobado' && esCC && (
                <div className="flex items-center gap-2 text-surface-400 text-xs font-body bg-surface-700 rounded-xl px-4 py-2.5">
                  <Clock size={13} />
                  El pago se registra desde <span className="text-white font-medium ml-1">Saldos</span>
                </div>
              )}
            </div>
          </div>
      ...
      {saldo && (
        <Card className="p-5">
          <div className="flex flex-wrap items-center gap-6">
            ...
            <div><p className="text-surface-400 text-xs uppercase tracking-widest font-body mb-1">Vence</p><p className="text-white font-body text-sm">{fmtFecha(saldo.fechaFin)}</p></div>
      ...
      {[
        { key: 'aprobar',  title: 'Aprobar presupuesto',  body: `El presupuesto #${pres.idPresupuesto} pasará a estado Aprobado y se descontará el stock correspondiente.${esCC ? ' Se generará un saldo pendiente de cobro.' : ''}`, action: () => cambiarEstado('aprobado'), btnLabel: 'Aprobar', btnIcon: ThumbsUp },
        { key: 'pagar',    title: 'Registrar pago',       body: `El presupuesto #${pres.idPresupuesto} se marcará como Pagado. El ingreso de ${fmt(pres.monto)} se reflejará en estadísticas.`, action: () => cambiarEstado('pagado'),   btnLabel: 'Confirmar pago', btnIcon: CheckCircle2 },
        { key: 'rechazar', title: 'Rechazar presupuesto', body: `El presupuesto #${pres.idPresupuesto} se marcará como Rechazado. Esta acción no afecta el stock.`, action: () => cambiarEstado('rechazado'), btnLabel: 'Rechazar', btnIcon: XCircle, danger: true },
      ].map(m => (
        <Modal key={m.key} open={modal === m.key} onClose={() => { setModal(null); setErrorModal('') }} title={m.title} width="max-w-sm">
          ...
          {errorModal && (
            <div className="flex items-center gap-2 text-red-400 text-xs bg-red-500/10 border border-red-500/30 rounded-xl px-3 py-2 mb-4">
              <AlertCircle size={13} className="flex-shrink-0" />{errorModal}
            </div>
          )}
          <div className="flex gap-2">
            <Button variant="secondary" className="flex-1" onClick={() => { setModal(null); setErrorModal('') }}>Cancelar</Button>
            <Button variant={m.danger ? 'danger' : 'primary'} className="flex-1" icon={m.btnIcon} onClick={m.action}>{m.btnLabel}</Button>
          </div>
        </Modal>
      ))}
    </div>
...
export default function Historial() {
  ...
  const load = useCallback(() => {
    ...
    setPage(1)
  }, [search, filterMetodo, filterEstado, filterFechaD, filterFechaH, sortDir])
    ...
    <div className="max-w-7xl mx-auto space-y-6">
      ...
      <Card className="p-4">
        <div className="flex flex-wrap gap-3 items-end">
          ...
          {(search||filterMetodo!=='all'||filterEstado!=='all'||filterFechaD||filterFechaH) && (
            <button onClick={()=>{setSearch('');setFilterMetodo('all');setFilterEstado('all');setFilterFechaD('');setFilterFechaH('')}}
              ...
              {paginated.map(p => {
                ...
                return (
                  <tr key={p.idPresupuesto} onClick={()=>setSelected(p)}
                    ...
                    <td className="py-3 px-4"><Badge color={BADGE[m.badge]}>{m.label}</Badge></td>
                    <td className="py-3 px-4">
                      <span className={`inline-flex items-center gap-1 text-xs font-body`}>
                        <EIcon size={11} className={`text-${e.color === 'gray' ? 'surface-400' : e.color+'-400'}`} />
